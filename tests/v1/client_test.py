...
from devices.v1.client import DevicesV1API
from devices.v1.query import CustomerDevices, Query
from requests import Session


def test_devices_session_creation_success(url, auth_token):
    # Given / When
    with DevicesV1API(url, auth_token) as devices:
...
def test_devices_session_invalid_token(url):
    with pytest.raises(InvalidTokenError):
        with DevicesV1API(url, auth_token=None) as _:
            pass
...
def test_get_get_devices(url, auth_token):
    ...
    # Then
    assert isinstance(customer_devices, CustomerDevices)
    assert customer_devices._session == devices._session
    assert customer_devices._url == devices._url
    assert customer_devices._query_parameters == {}
